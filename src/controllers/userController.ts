...
export const updateUser = async (request: Request, response: Response) => {
  try {
    ...
    const findUser = await prisma.user.findFirst({
      where: { idUser: { equals: Number(idPenjual) } },
    });
    if (!findUser)
    ...
    if (request.file) {
      // Ensure request.file is properly handled
      ...
      const path = `${BASE_URL}/public/profile/${findUser.profile}`;
      const exists = fs.existsSync(path);

      // Delete old photo if a new one is uploaded
      if (exists && findUser.profile !== "") fs.unlinkSync(path);
    }
    ...
    const updateUser = await prisma.user.update({
      data: {
        ...
        profile: filename || findUser.profile,
      },
...
export const penjualPicture = async (
  request: Request,
  response: Response
): Promise<void> => {
  try {
    const { idPenjual } = request.params;
    const findPenjual = await prisma.user.findFirst({
      where: { idUser: { equals: parseInt(idPenjual) } },
    });
    if (!findPenjual) {
      response.status(200).json({
        status: false,
        massage: `user with id ${idPenjual} is not found`,
      });
      return;
    }
    ...
    if (request.file) {
      ...
      // Check for old photo in folder
      const path = `${BASE_URL}../public/profile/${findPenjual.profile}`;
      const exists = fs.existsSync(path);
